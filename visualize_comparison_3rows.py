#!/usr/bin/env python3
"""
MDAE 3-Row Comparison Visualization Script

This script generates a 3×6 grid comparison showing different corruption strategies:
...
Usage:
    # Generate 3-row comparison with default settings
    python visualize_comparison_3rows.py --dataset brats18 --modality t1ce

    # Use specific case
    python visualize_comparison_3rows.py --case-id Brats18_2013_0_1

    # High-resolution output
    python visualize_comparison_3rows.py --dpi 600

    # Use synthetic data for testing
    python visualize_comparison_3rows.py --use-synthetic
...
from pyvista_utils import create_comparison_3rows


def main():
    parser = argparse.ArgumentParser(
        description="Generate 3-row comparison: MAE vs Diffusion vs MDAE"
    )
    parser.add_argument(
        '--sample-number',
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.75,
        help='Masking percentage for MAE row (default: 0.75)'
    )
    parser.add_argument(
        '--sde',
        type=str,
    ...
    # Setup
    print("=" * 80)
    print("MDAE 3-Row Comparison Visualization")
    print("=" * 80)
    ...
    # Generate 3-row comparison
    print(f"\n2. Generating 3-row comparison visualization...")
    print(f"   Grid: 3 rows × {1 + args.sample_number} columns")
    ...
    print(f"   Row 3 (MDAE): Dual corruption ({args.corruption_min}-{args.corruption_max})")
    print(f"   Patch size: {args.patch_size}³")
    print(f"   SDE type: {args.sde}")
    print(f"   Transparency: {args.transparency}")

    # Define save path
    filename = f"MDAE_Comparison_3rows_{case_name}_{modality_name}_{args.sample_number}samples.png"
    ...
    plotter = create_comparison_3rows(
        volume,
    ...
    # Note: save_publication_image is called inside create_comparison_3rows
    plotter.close()

    # Summary
    print("\n" + "=" * 80)
    print("✓ 3-row comparison visualization generated successfully!")
    print("=" * 80)
    print(f"\nOutput file:")
    ...
    print(f"  - Row 3: MDAE (dual {args.corruption_min}-{args.corruption_max})")
    print(f"  - Patch size: {args.patch_size}³")
    print(f"  - SDE type: {args.sde}")
    print(f"  - Transparency: {args.transparency}")
    print(f"  - Image DPI: {args.dpi}")

    print("\n3-row comparison ready for publication!")
